 ...
 * created
 *
 * Returns in \p *ppD3D10Device the Direct3D device against which this CUDA
 * context was created in ::cudaD3D10SetDirect3DDevice().
 *
 ...
 *
 * If Direct3D interoperability is not initialized using ::cudaD3D10SetDirect3DDevice then
 ...
 * a CUDA device
 *
 * Records \p pD3D10Device as the Direct3D 10 device to use for Direct3D 10
 ...
 * to reset \p device using ::cudaDeviceReset() before Direct3D 10 
 * interoperability on \p device may be enabled.
 *
 * Successfully initializing CUDA interoperability with \p pD3D10Device 
 * will increase the internal reference count on \p pD3D10Device.  This 
